...
public class FileMessageSet implements MessageSet, Closeable {
    ...
    private void tryToLogTransferInfo(final GetCommand getCommand, final Connection conn) {
        if (transferLog.isDebugEnabled()) {
			...
			// topic
			sb.append("T:").append(getCommand.getTopic()).append(" ");
			// partition
			sb.append("P:").append(getCommand.getPartition()).append(" ");
			// group
			sb.append("G:").append(getCommand.getGroup()).append(" ");
			// offset
			sb.append("O:").append(this.offset).append(" ");
			// sizeInBytes
			sb.append("S:").append(this.sizeInBytes.get()).append(" ");
			// client address
			final String addrString = conn != null ? RemotingUtils
					.getAddrString(conn.getRemoteSocketAddress()) : "unknown";
