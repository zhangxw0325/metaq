...
import com.taobao.metaq.store.MetaQueueLogistic;
...
public class StatsManager implements Service {
    ...
    public StatsManager(final MetaConfig metaConfig, final MessageStoreManager messageStoreManager,
            final RemotingServer remotingServer,final MetaStore metaStore) {
        super();
    ...
    public String getAskInfo(String topic, String type, String[] args) {
    	assert(null != topic);
    	assert(null != type);
    	final StringBuilder sb = new StringBuilder(1024);
        sb.append("ASK\r\n");
        if ("offsets".equals(type)) {
        	sb.append(getAskOffsetsInfo(topic, type, args));
        }
        sb.append("END\r\n");
        return sb.toString();
    }
    ...
    private String getAskOffsetsInfo(String topic, String type, String[] args) {
    	assert(null != args);
    	final StringBuilder sb = new StringBuilder(1024);
    	long timestamp=Long.parseLong(args[0]);
    		
    	//FIXME bad practise
    	DefaultMetaStore dms = (DefaultMetaStore)metaStore;
    	final ConcurrentHashMap<String/* topic */, ConcurrentHashMap<Integer/* queueId */, MetaQueueLogistic>> stores = 
    			dms.getMetaQueueLogisticTable();
    	final ConcurrentHashMap<Integer, MetaQueueLogistic> queueMap=stores.get(topic);
        if(null != queueMap) {
        	for(Map.Entry<Integer, MetaQueueLogistic> entry : queueMap.entrySet()) {
        		int queueId=entry.getKey();
        		long offset=metaStore.getOffsetInQueueByTime(topic, queueId, timestamp);
        		this.append(sb, topic, "part", queueId, offset);
        	}
        }
        return sb.toString();
    }
    ...
    private void appendStore(final StringBuilder sb) {
        sb.append(this.metaStore.getRunningDataInfo());
    }
    ...
    private void appendOffsetInfo(final StringBuilder sb) {
    	...
    	final ConcurrentHashMap<String/* topic */, ConcurrentHashMap<Integer/* queueId */, MetaQueueLogistic>> stores = 
    			dms.getMetaQueueLogisticTable();
    	for (final Map.Entry<String, ConcurrentHashMap<Integer, MetaQueueLogistic>> entry : stores.entrySet()) {
    		final String topic = entry.getKey();
    		final ConcurrentHashMap<Integer, MetaQueueLogistic> subMap = entry.getValue();
    		if (subMap != null) {
    			for (final Map.Entry<Integer, MetaQueueLogistic> subEntry : subMap.entrySet()) {
    				final int queueId = subEntry.getKey();
    				final MetaQueueLogistic store = subEntry.getValue();
    				this.append(sb, topic, "part", queueId, "min_offset", store.getMinOffsetInQuque(), "max_offset", 
    					store.getMaxOffsetInQuque());
    			}
    		}
    	}
    }
