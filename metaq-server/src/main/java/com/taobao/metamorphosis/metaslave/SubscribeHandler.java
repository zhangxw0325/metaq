...
public class SubscribeHandler implements SubscribeHandlerMBean {
    ...
    public SubscribeHandler(final MetaMorphosisBroker broker) throws MetaClientException {
        ...
        this.slaveZooKeeper = null;
        final MetaConfig metaConfig = this.broker.getMetaConfig();

        ZKConfig zkConfig = metaConfig.getZkConfig();
        if (zkConfig != null && zkConfig.zkConnect == null) {
            zkConfig = null;
        }
    ...
    synchronized public void start() {
        ...
        try {
            ...
            log.warn(
                "problem occured in start subscribe message from master,maybe master not started or other errers",
                e);
                ...
                for (final Partition partition : each.getValue()) {
                    ...
                    if (messageStore == null) {
                        ...
                        final long offsetInMaster =
                                this.slaveOffsetStorage
                                    .queryOffsetInMaster(this.masterServerUrl, partition, topic);
    ...
    private void initMessageConsumer() throws Exception {
        if (this.consumer == null) {
            ConsumerConfig consumerConfig = new ConsumerConfig(this.broker.getMetaConfig().getSlaveGroup());
