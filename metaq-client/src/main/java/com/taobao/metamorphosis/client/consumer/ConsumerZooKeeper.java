...
public class ConsumerZooKeeper implements ZkClientChangedListener {
    protected volatile ZkClient zkClient;
            ...
            if (futureTask != null) {
                ...
                if (listener != null) {
                    ...
                    final ZKGroupDirs dirs =
                            this.metaZookeeper.new ZKGroupDirs(listener.consumerConfig.getGroup());
                    ...
                    // 移除standby结点监听
                    this.zkClient.unsubscribeDataChanges(dirs.consumerStandbyDir, new MasterSlaveRetrieveListener(listener));
                }
            }
        }
    ...
    protected ZKLoadRebalanceListener registerConsumerInternal(final ZKLoadRebalanceListener loadBalanceListener)
        ...
        if (this.zkClient == null) {
            ...
            // 监控主备切换状态（该值控制客户端从主机还是备机拉取数据）
            // 不用显示的将consumerStandbyDir节点注册到zk。通过手动的更改zk这个结点的值来控制客户端是从主机还是备机拉取数据
            this.zkClient.subscribeDataChanges(dirs.consumerStandbyDir, 
            		new MasterSlaveRetrieveListener(loadBalanceListener));
            
            // 注册客户端版本信息
            ZkUtils.updatePersistentPath(this.zkClient, dirs.consumerVersionDir, "2.1.3");
    ...
    class MasterSlaveRetrieveListener implements IZkDataListener {
    	private final ZKLoadRebalanceListener loadBalancerListener;
    	
    	
    	public MasterSlaveRetrieveListener(ZKLoadRebalanceListener loadBalancerListener) {
		...
		public void handleDataChange(String dataPath, Object data) throws Exception {
			loadBalancerListener.switchRetrieveFrom();
		}
		

		@Override
		public void handleDataDeleted(String dataPath) throws Exception {
			log.info(dataPath + " date have been deleted.");
			loadBalancerListener.switchRetrieveFrom();
		}
    }
    ...
    protected class ZKLoadRebalanceListener implements IZkChildListener {
        ...
        static final int MAX_N_RETRIES = 20;
        ...
        public ZKLoadRebalanceListener(final FetchManager fetchManager, final ZKGroupDirs dirs,
                final String consumerIdString, final ConsumerConfig consumerConfig,
        ...
        private long retryInterval() {
            long result = ConsumerZooKeeper.this.zkConfig.zkSyncTimeMs;
        ...
        void syncedRebalance() throws Exception {
            ...
            try {
                for (int i = 0; i < MAX_N_RETRIES; i++) {
                    ...
                    this.releaseAllPartitionOwnership2();
            ...
            // ZK CLient CLOSE
                ...
                log.warn("zk client closed, so end current reblance");
            }
            finally {
        ...
        private Broker getBroker(Cluster cluster, Partition partition) {
        	String flag=ZkUtils.readDataMaybeNull(ConsumerZooKeeper.this.zkClient, this.dirs.consumerStandbyDir);
        	int whichServer;
        	if(null == flag || "".equals(flag) || "null".equals(flag))
        		whichServer = this.consumerConfig.getPullFromWhichServer();
        	else
        		whichServer = Integer.parseInt(flag);
        	
        	return cluster.selectWhichBroker(partition.getBrokerId(), whichServer);
        }
        ...
        protected void updateFetchRunner(final Cluster cluster) throws Exception {
            ...
            for (final Map.Entry<String/* topic */, ConcurrentHashMap<Partition, TopicPartitionRegInfo>> entry : this.topicRegistry
                ...
                for (final Map.Entry<Partition, TopicPartitionRegInfo> partEntry : entry.getValue().entrySet()) {
                    ...
                    // master与slave选择由用户配置
                    final Broker broker = getBroker(cluster, partition);
                    if (broker != null) {
            ...
            final int serverConnectionCount = this.consumerConfig.getConsumerServerConnectionCount();

            // 建立连接
            for (final Broker broker : changedBrokers) {
                if (!this.oldBrokerSet.contains(broker)) {
                    try {
                        ConsumerZooKeeper.this.remotingClient.connect(broker.getZKString(), serverConnectionCount);
        ...
        protected Map<String, List<String>> getPartitionStringsForTopics(
            ...
            return ConsumerZooKeeper.this.metaZookeeper.getPartitionStringsForTopics(myConsumerPerTopicMap
                .keySet());
        }


        void switchRetrieveFrom() throws Exception {
        	this.rebalanceLock.lock();
        	try {
        		log.warn("Stopping fetch runners, retrieve from master or slave changed");
        		this.fetchManager.stopFetchRunner();
        		this.updateFetchRunner(this.oldCluster);
        		log.warn("Start fetch runners success, retrieve from master or slave changed");
        	}
        	finally {
        		this.rebalanceLock.unlock();
        	}
        ...
        private void releaseAllPartitionOwnership() {
            for (final Map.Entry<String, ConcurrentHashMap<Partition, TopicPartitionRegInfo>> entry : this.topicRegistry
                ...
                final String topic = entry.getKey();
                final ZKGroupTopicDirs topicDirs =
                        ConsumerZooKeeper.this.metaZookeeper.new ZKGroupTopicDirs(topic,
        ...
        private void releaseAllPartitionOwnership2() throws InterruptedException {
            for (final Map.Entry<String, ConcurrentHashMap<Partition, TopicPartitionRegInfo>> entry : this.topicRegistry
                .entrySet()) {
                final String topic = entry.getKey();
                final ZKGroupTopicDirs topicDirs =
        ...
        private void deleteOwnership2(final String znode) throws InterruptedException {
            try {
                ZkUtils.deletePath(ConsumerZooKeeper.this.zkClient, znode);
            }
            // ZK CLient CLOSE
            catch (InterruptedException e) {
                log.warn("deleteOwnership2 has InterruptedException exception, throw it");
                throw e;
            }
            catch (final Throwable t) {
                log.error("exception during releasePartitionOwnership", t);
         ...
         * @throws InterruptedException
         */
        private void releasePartitionOwnership(final String topic) throws InterruptedException {
            final ZKGroupTopicDirs topicDirs =
                    ConsumerZooKeeper.this.metaZookeeper.new ZKGroupTopicDirs(topic,
                        this.consumerConfig.getGroup());
