...
public class SimpleMessageProducer implements MessageProducer, TransactionSession {
    ...
    protected static final long DEFAULT_OP_TIMEOUT = 10000L;
    private static final int TIMEOUT_THRESHOLD = Integer.parseInt(System.getProperty(
        "meta.send.timeout.threshold", "200"));
    protected final MetaMessageSessionFactory messageSessionFactory;
    ...
    protected SendResult sendMessageToServer(final Message message, final long timeout, final TimeUnit unit)
        ...
        final boolean compressMessage = this.messageSessionFactory.getMetaClientConfig().isCompressMessage();
        final int compressLevel = this.messageSessionFactory.getMetaClientConfig().getCompressLevel();

        try {
            final byte[] data = SimpleMessageProducer.encodeData(message, compressMessage, compressLevel);
            ...
            throw new MetaClientException("compress message error", e);
        }
        finally {
    ...
    private SendResult send0(final Message message, final byte[] encodedData, final long timeout,
            final TimeUnit unit) throws InterruptedException, MetaClientException {
        ...
        try {
            ...
            if (this.isInTransaction()) {
                ...
                if (info != null) {
                    ...
                    // 选择该broker内的某个分区
                    partition =
                            this.producerZooKeeper.selectPartition(topic, message, this.partitionSelector,
                                serverUrl);
                    if (partition == null) {
                        // 没有可用分区，抛出异常
            ...
            final boolean compressMessage = this.messageSessionFactory.getMetaClientConfig().isCompressMessage();
            flag = MessageFlagUtils.getFlagCompress(flag, compressMessage);
            ...
            final BooleanCommand resp =
                    this.invokeToGroup(serverUrl, partition, putCommand, message, timeout, unit);
            ...
            throw new MetaOpeartionTimeoutException("Send message timeout in "
                    + TimeUnit.MILLISECONDS.convert(timeout, unit) + " mills, partition is:" + partition
                    + ", server url is:" + serverUrl, e);
        }
        catch (final InterruptedException e) {
            throw e;
        }
        catch (final Exception e) {
            throw new MetaClientException("send message failed, " + e.getMessage(), e);
        }
    }
    ...
    private SendResult genSendResult(final Message message, final Partition partition, final String serverUrl,
        ...
        case HttpStatus.Success: {
            ...
            MessageAccessor.setMsgNewId(message, msgNewId);
     ...
     * @throws IOException
     */
    public static byte[] encodeData(final Message message, final boolean compress, final int level)
            throws IOException {
        byte[] payload = null;
        if (compress) {
            payload = MetaUtil.compress(message.getData(), level);
        }
        else {
    ...
    @Override
    public void sendMessage(final Message message, final SendMessageCallback cb, final long time,
            final TimeUnit unit) {
        try {
            final String topic = message.getTopic();
            ...
            final byte[] encodedData = SimpleMessageProducer.encodeData(message, compressMessage, compressLevel);
            final PutCommand putCommand =
                    new PutCommand(topic, partition.getPartition(), encodedData, this.getTransactionId(), flag,
                        OpaqueGenerator.getNextOpaque());
