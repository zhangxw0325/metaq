...
public class ProducerZooKeeper implements ZkClientChangedListener {
    ...
    final class BrokerConnectionListener implements IZkChildListener {
        ...
        void syncedUpdateBrokersInfo() throws NotifyRemotingException, InterruptedException {
            ...
            try {
                ...
                final int serverConnectionCount = ProducerZooKeeper.this.metaClientConfig.getProducerServerConnectionCount();
                ...
                for (final Map.Entry<Integer, String> oldEntry : this.brokersInfo.oldBrokerStringMap.entrySet()) {
                    ...
                    if (newBrokerStringMap.containsKey(oldBrokerId)) {
                        ...
                        if (!newBrokerString.equals(oldBrokerString)) {
                            ...
                            ProducerZooKeeper.this.remotingClient.connect(newBrokerString, serverConnectionCount);
    ...
    public ProducerZooKeeper(final MetaZookeeper metaZookeeper, final RemotingClientWrapper remotingClient, final ZkClient zkClient,
     ...
     * @return
     */
    Partition selectPartition(final String topic, final Message msg, final PartitionSelector selector, final String serverUrl)
     ...
     * @return 选中的分区
     */
    public Partition selectPartition(final String topic, final Message message, final PartitionSelector partitionSelector)
            throws MetaClientException {
        if (this.metaClientConfig.getServerUrl() != null) {
    ...
    private Partition selectDefaultPartition(final String topic, final Message message, final PartitionSelector partitionSelector,
