/**
 * $Id: MapedFileQueue.java 3 2013-01-05 08:20:46Z shijia $
 */
            ...
            for (File file : files) {
                ...
                if (file.length() != this.mapedFileSize) {
                    log.warn(file + "\t" + file.length()
                            + " length not matched meta store config value, ignore it");
                    return true;
                }
        ...
        if (null != mfs) {
            ...
            for (int i = 0; i < mfsLength; i++) {
                ...
                SelectMapedBufferResult result =
                        mapedFile.selectMapedBuffer(this.mapedFileSize - MetaQueueLogistic.StoreUnitSize);
                if (result != null) {
