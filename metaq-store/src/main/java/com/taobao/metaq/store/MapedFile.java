/**
 * $Id: MapedFile.java 3 2013-01-05 08:20:46Z shijia $
 */
...
public class MapedFile extends ReferenceResource {
    ...
    public AppendMessageResult appendMessage(final Object msg, final AppendMessageCallback cb) {
        ...
        // 上层应用应该保证不会走到这里
        log.fatal("MapedFile.appendMessage return null, wrotePostion: " + currentPos + " fileSize: "
                + this.fileSize);
    ...
    public boolean cleanup(final long currentRef) {
        ...
        if (this.isAvailable()) {
            log.fatal("this file[REF:" + currentRef + "] " + this.fileName + " have not shutdown, stop unmaping.");
            return false;
        }

        // 如果已经cleanup，再次操作会引起crash
        if (this.isCleanupOver()) {
            log.fatal("this file[REF:" + currentRef + "] " + this.fileName + " have cleanup, do not do it again.");
            // 必须返回true
            return true;
        }
