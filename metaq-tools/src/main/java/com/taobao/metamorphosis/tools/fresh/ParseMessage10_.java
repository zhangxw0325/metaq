...
public class ParseMessage10_ 
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		{
			byte[] data=null;
			while((data=parseMessage(raf))!=null)
			{
	...
	public static byte[] parseMessage(RandomAccessFile raf) throws Exception
	{
		byte[] ret=null;
		byte[] header=new byte[20];
		int index=-1;
		while((index=raf.read(header))!=-1)
		{
			...
			if((flag & 0x1)==1)
			 {
				 int attributeLen=raf.readInt();
				 byte[] temp=new byte[attributeLen];
				 raf.read(temp);
			 }
