            ...
            return new Decoder() {
                ...
                public Object decode(final IoBuffer buff, final Session session) {
                    ...
                    if (index >= 0) {
                        ...
                        case 'a':
                        	return this.decodeAsk(tmps);
                        case 'f':
                        	return this.decodeFetch(tmps);
                        case 'm':
                        	return this.decodeMessageType(buff, tmps);
                        default:
                            throw new MetaCodecException("Unknow command:" + line);
                        }
                    }
                ...
                private Object decodeMessageType(final IoBuffer buf, final String[] tmps){
                	this.assertCommand(tmps[0], "messageType");
                	int len = Integer.parseInt(tmps[6]);
                	...
                	String[] types = StringUtils.split(value, MessageTypeCommand.SEP);
                	...
                	return new MessageTypeCommand(tmps[1], tmps[2], tmps[3], Integer.parseInt(tmps[4]), typeSet, Long.parseLong(tmps[5]));
                }
                
                private Object decodeFetch(final String[] tmps){
                	this.assertCommand(tmps[0], "fetch");
                    return new FetchCommand(tmps[1], tmps[2], tmps[3], Integer.parseInt(tmps[4]), 
                    		Long.parseLong(tmps[5]), Integer.parseInt(tmps[6]), Integer.parseInt(tmps[7]), Long.parseLong(tmps[8]));
                }
                ...
                private Object decodeAsk(final String[] tmps) {
                	assert(tmps.length >= 3);
                	this.assertCommand(tmps[0], "ask");
                	if (tmps.length == 3)
                		return new AskCommand(tmps[1], tmps[2], null);
                	String[] params=new String[tmps.length-3];
                	System.arraycopy(tmps, 3, params, 0, tmps.length-3);
                	return new AskCommand(tmps[1], tmps[2], params);
                }
